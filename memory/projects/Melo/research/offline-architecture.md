# Offline-First Architecture for MELO v2
...
MELO v2 requires robust offline-first architecture to provide seamless messaging across unreliable network conditions. This document analyzes storage strategies, sync patterns, and Matrix SDK configuration for true offline support.
...
| `IndexedDBStore` | Persistent IndexedDB storage | **Recommended for MELO** |
...
MELO should use multiple caching strategies based on resource type:
...
class MeloDatabase extends Dexie {
...
class IndexedDBAdapter implements StorageAdapter {
  private db: MeloDatabase;
  
  async init() {
    this.db = new MeloDatabase();
...
class SQLiteAdapter implements StorageAdapter {
  ...
  async init() {
    ...
    this.db = await openDatabase('melo.db');
...
**For IndexedDB (Web):**
```typescript
import { encrypt, decrypt } from '@melo/crypto';
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    MELO Offline Sync Architecture                    │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│                          ┌─────────────┐                            │
│                          │  UI Layer   │                            │
