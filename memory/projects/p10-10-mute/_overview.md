...
## Task Summary
Implement server/channel mute functionality for users with timed mutes in HAOS v2.
...
- **Added `muteUser()` method:**
  ...
  - Stores mute information in Matrix room state (`org.haos.moderation.mute`)
...
- **State Storage:** Mute data stored in room state events (`org.haos.moderation.mute`)
