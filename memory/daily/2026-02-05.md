...
- **Fix:** 
  - RandOffset pipelines → `StandardPositionalOffsetAttacker`
  - RandomPos → use `add_attacks_positional_override_rand` (correct method)
  - PositionSwap → `StandardPositionalOffsetAttacker` (has swap method)
  - Train/test split → 80/20 proportional instead of hardcoded 100K
- **Commit:** `3472a89` — "fix: Correct attacker classes + proportional train/test split"
