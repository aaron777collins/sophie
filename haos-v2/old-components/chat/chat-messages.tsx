...
import { useLongPress, useSwipeGestures, usePullToRefresh } from '../../hooks/use-mobile-gestures';
...
export const ChatMessages: React.FC<ChatMessagesProps> = ({
  ...
  const { pullToRefreshHandlers, pullRefreshIndicator } = usePullToRefresh({
  ...
  const handleReaction = useCallback((emoji: string) => {
    if (selectedMessage) {
      onReactionAdd?.(selectedMessage.id, emoji);
    }
  }, [selectedMessage, onReactionAdd]);
  ...
  const handleEdit = useCallback(() => {
    if (selectedMessage) {
      // For editing, you might want to trigger an edit mode
