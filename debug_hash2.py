...
for radius_name in ["2km", "100km"]:
    ...
    # Debug: Check the model name being used
    model_name = runner._generatorPathProvider.getModelName()
    print(f"config_hash: {hash_val}")
    print(f"model_name in provider: {model_name}")
    print(f"id(runner._generatorPathProvider): {id(runner._generatorPathProvider)}")
    
    # Check the lambda stored in contexts
    path_lambda = runner._generatorPathProvider.get("ConnectedDrivingLargeDataCleaner.combinedcleandatapath")
    print(f"path_lambda: {path_lambda}")
