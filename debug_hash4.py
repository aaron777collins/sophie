...
print(f"runner1.id: {id(runner1)}, provider.id: {id(runner1._generatorPathProvider)}")
print(f"runner1 model_name: {runner1._generatorPathProvider.getModelName()}")

print("\n=== Creating second runner (100km) ===")
config2 = generate_pipeline_config("basic", "100km", "rand_offset")
runner2 = DaskPipelineRunner(config2)
print(f"runner2.id: {id(runner2)}, provider.id: {id(runner2._generatorPathProvider)}")
print(f"runner2 model_name: {runner2._generatorPathProvider.getModelName()}")

print("\n=== Final comparison ===")
print(f"runner1 path: {runner1._generatorPathProvider.getPathWithModelName('ConnectedDrivingLargeDataCleaner.combinedcleandatapath')}")
print(f"runner2 path: {runner2._generatorPathProvider.getPathWithModelName('ConnectedDrivingLargeDataCleaner.combinedcleandatapath')}")
