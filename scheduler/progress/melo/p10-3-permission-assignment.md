...
Implement granular permission toggles with Admin/Moderator/Member base templates and Matrix power level integration for MELO Full Implementation.
...
- **Matrix Integration:** Maps MELO permissions to Matrix power levels and state events
...
MATRIX_PERMISSION_MAPPINGS: Record<keyof MeloPermissions, MatrixPermissionMapping[]>
```
...
```typescript
calculateRequiredPowerLevel(permissions: MeloPermissions): number
```
...
2. **Matrix Client:** Permissions stored in room account data as `dev.melo.custom_roles`
...
- [x] **Build passes (`cd ~/repos/melo-v2 && pnpm build`)** ✅
...
- Permission system is extensible - easy to add new permissions by updating the MeloPermissions interface and MATRIX_PERMISSION_MAPPINGS
