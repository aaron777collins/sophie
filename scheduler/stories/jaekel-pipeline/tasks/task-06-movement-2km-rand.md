...
| Property | Value |
|----------|-------|
| **Pipeline Name** | movement_2km_rand |
| **Config File** | `production_configs_v2/movement_2km_rand_pipeline_config.json` |
...
# Execute pipeline
python production_configs_v2/Run2kmMovementRand.py 2>&1 | tee logs/movement_2km_rand_$(date +%Y%m%d_%H%M%S).log
```

### Step 2: Log Audit

```bash
LOG=$(ls -t logs/movement_2km_rand_*.log | head -1)

echo "=========================================="
echo "LOG AUDIT: movement_2km_rand"
echo "Log file: $LOG"
echo "=========================================="
...
ls -la results/matrix/movement_2km_rand/
# EXPECTED: CSV files with ML results

# Check cache created
ls -la cache/matrix/movement_2km_rand/
...
- [ ] ML accuracy > 65%
- [ ] Results files created in results/matrix/movement_2km_rand/
- [ ] Cache files created
...
3. Proceed to Task 07 (unless this is Task 36)
