...
| Property | Value |
|----------|-------|
| **Pipeline Name** | basic_2km_const |
| **Config File** | `production_configs_v2/basic_2km_const_pipeline_config.json` |
...
# Execute pipeline
python production_configs_v2/Run2kmBasicConst.py 2>&1 | tee logs/basic_2km_const_$(date +%Y%m%d_%H%M%S).log
```

### Step 2: Log Audit

```bash
LOG=$(ls -t logs/basic_2km_const_*.log | head -1)
...
ls -la results/matrix/basic_2km_const/
# EXPECTED: CSV files with ML results

# Check cache created
ls -la cache/matrix/basic_2km_const/
...
- [ ] ML accuracy > 65%
- [ ] Results files created in results/matrix/basic_2km_const/
- [ ] Cache files created
...
3. Proceed to Task 02 (unless this is Task 36)
