...
| Property | Value |
|----------|-------|
| **Pipeline Name** | extended_200km_const |
| **Config File** | `production_configs_v2/extended_200km_const_pipeline_config.json` |
| **Run Script** | `production_configs_v2/Run200kmExtendedConst.py` |
| **Radius** | 200km |
| **Feature Set** | Extended (6 features) |
| **Vehicle ID** | No |
...
# Execute pipeline
python production_configs_v2/Run200kmExtendedConst.py 2>&1 | tee logs/extended_200km_const_$(date +%Y%m%d_%H%M%S).log
```

### Step 2: Log Audit

```bash
LOG=$(ls -t logs/extended_200km_const_*.log | head -1)

echo "=========================================="
echo "LOG AUDIT: extended_200km_const"
echo "Log file: $LOG"
echo "=========================================="
...
ls -la results/matrix/extended_200km_const/
# EXPECTED: CSV files with ML results

# Check cache created
ls -la cache/matrix/extended_200km_const/
...
- [ ] ML accuracy > 65%
- [ ] Results files created in results/matrix/extended_200km_const/
- [ ] Cache files created
...
3. Proceed to Task 34 (unless this is Task 36)
