...
| Property | Value |
|----------|-------|
| **Pipeline Name** | movement_100km_const |
| **Config File** | `production_configs_v2/movement_100km_const_pipeline_config.json` |
...
# Execute pipeline
python production_configs_v2/Run100kmMovementConst.py 2>&1 | tee logs/movement_100km_const_$(date +%Y%m%d_%H%M%S).log
```

### Step 2: Log Audit

```bash
LOG=$(ls -t logs/movement_100km_const_*.log | head -1)

echo "=========================================="
echo "LOG AUDIT: movement_100km_const"
echo "Log file: $LOG"
echo "=========================================="
...
ls -la results/matrix/movement_100km_const/
# EXPECTED: CSV files with ML results

# Check cache created
ls -la cache/matrix/movement_100km_const/
...
- [ ] ML accuracy > 65%
- [ ] Results files created in results/matrix/movement_100km_const/
- [ ] Cache files created
...
3. Proceed to Task 18 (unless this is Task 36)
