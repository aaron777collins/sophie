# HAOS v2 Implementation Plan

**Version:** 1.0  
**Date:** 2025-02-13  
**Status:** Final Audit Synthesis  
**Based On:** All HAOS v2 audit documents
...
HAOS v2 is a **Discord-styled Matrix client** built on Element Web 1.12.9. It combines:
- **Discord's familiar UI/UX** (servers, channels, message layout)
- **Matrix's protocol strengths** (federation, E2EE, decentralization)
- **HAOS-specific features** (self-hosting, privacy controls, enterprise options)
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        HAOS V2 IMPLEMENTATION ROADMAP                       │
└─────────────────────────────────────────────────────────────────────────────┘
...
| 0.5 Discord clone extraction | Copy relevant components to HAOS repo | 2 days | Frontend |
...
| 3.5.2 Video call styling | HAOS-themed call UI | 1 day |
...
| `io.haos.channel` | Channel type (TEXT/AUDIO/VIDEO) | `''` |
| `io.haos.channel_order` | Channel ordering in space | `''` |
| `io.haos.member_role` | Display role name | `{userId}` |
| `io.haos.profile` | Extended profile data | Account data |
...
docker compose logs -f haos      # View logs
...
# Optional
MATRIX_ADMIN_TOKEN=syt_xxx       # For admin operations
NEXT_PUBLIC_BRAND_NAME=HAOS      # Custom branding
```
