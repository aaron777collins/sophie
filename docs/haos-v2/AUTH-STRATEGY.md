# HAOS v2 Auth Strategy: Clerk → Matrix Authentication
...
This document details the complete migration strategy from Clerk authentication to Matrix native authentication for HAOS v2.
...
└─────────────────────────────────────────────────────────────────────────────┘
    ...
    User                    HAOS Client                   Matrix Homeserver
      │                          │                               │
      │  1. Enter credentials    │                               │
      │ ────────────────────────>│                               │
      │                          │                               │
...
|------------|-------------|----------|
| `m.login.password` | Username + password | Primary for HAOS |
...
| Concept | Description |
|---------|-------------|
| **User ID** | `@localpart:homeserver` (e.g., `@alice:haos.example.com`) |
...
Request:
{
  "username": "alice",
  "password": "secret123",
  "initial_device_display_name": "HAOS Web"
}
...
│                    MATRIX USER                           │
│  ────────────────────────────────────────────────────    │
│  userId: @alice:haos.example.com                         │
│                                                          │
│  Profile Data (m.room.member in rooms):                  │
│    - displayname: "Alice"                                │
│    - avatar_url: "mxc://haos.example.com/abc123"         │
│                                                          │
│  Account Data (io.haos.profile):                         │
...
export function MatrixAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const restoreSession = async () => {
      try {
        ...
        const accountData = await matrixClient.getAccountDataFromServer('io.haos.profile')
          .catch(() => null);
  ...
  const register = useCallback(async (
    ...
    await matrixClient.setAccountData('io.haos.profile', {
...
export default function SignInPage() {
  ...
  const [homeserver, setHomeserver] = useState(
    process.env.NEXT_PUBLIC_MATRIX_HOMESERVER || 'https://matrix.haos.local'
  );
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#313338]">
      <div className="w-full max-w-md p-8 space-y-6 bg-[#2b2d31] rounded-lg shadow-xl">
        <div className="text-center">
          ...
          <p className="text-zinc-400 mt-2">Sign in to continue to HAOS</p>
        </div>
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#313338]">
      <div className="w-full max-w-md p-8 space-y-6 bg-[#2b2d31] rounded-lg shadow-xl">
        <div className="text-center">
          ...
          <p className="text-zinc-400 mt-2">Join HAOS today</p>
        </div>
...
export async function getCurrentMatrixUser(): Promise<MatrixProfile | null> {
  ...
  try {
    ...
    const accountData = await client.getAccountDataFromServer('io.haos.profile')
      .catch(() => null);
...
```env
# Matrix Homeserver
NEXT_PUBLIC_MATRIX_HOMESERVER=https://matrix.haos.local
MATRIX_HOMESERVER_URL=https://matrix.haos.local

# Optional: SSO Configuration (if using external IdP)
MATRIX_OIDC_ISSUER=https://auth.example.com
MATRIX_OIDC_CLIENT_ID=haos-web
